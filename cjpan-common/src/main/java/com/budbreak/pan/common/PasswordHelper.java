...
public class PasswordHelper {
    ...
    @Getter
    @Setter
    private static String algorithName = SysConstant.ALGORITHNAME;
    //散列迭代次数
    @Getter
    @Setter
    private static int hashInterations = SysConstant.HASHNUM;
    ...
    public  static void encryptPassword(User user) {
        if (user.getPassword() != null) {
            ...
            if (user.getAlias()==null) {
                user.setAlias(randomNumberGenerator.nextBytes().toHex());
            }
            //调用SimpleHash指定散列算法参数：1、算法名称；2、用户输入的密码；3、盐值（随机生成的）；4、迭代次数
            String newPassword = new SimpleHash(
                    algorithName,
                    user.getPassword(),
                    ByteSource.Util.bytes(user.getAlias()),
